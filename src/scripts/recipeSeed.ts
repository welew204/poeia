...
const createNegroni = async () => {
    ...
    const firstStep = await db.step.create({
        data: {
            action: "Combine equal parts of gin, sweet vermouth, and Campari in a mixing glass with ice. Stir until chilled. ",
            stepNumber: 1,
            createdAt: new Date(),
            updatedAt: new Date(),
            recipe: {
    ...
    const secondStep = await db.step.create({
        data: {
            action: "Pour into a lowball glass over ice or straight up, then garnish with a piece of orange peel.",
            stepNumber: 1,
            createdAt: new Date(),
            updatedAt: new Date(),
            recipe: {
...
const createGnT = async () => {
    ...
    const stepTwo = await db.step.create({
        data: {
            action: "Top with the tonic water and gently stir. Garnish with lime wheels.",
            stepNumber: 2,
            createdAt: new Date(),
            updatedAt: new Date(),
            recipe: {
...
const createLemonDrop = async () => {
    ...
    const firstStep = await db.step.create({
        data: {
            action: "Add sugar to a plate, then rub the zest into the sugar with your fingers until it is tinted yellow and fragrant.",
            stepNumber: 1,
            createdAt: new Date(),
            updatedAt: new Date(),
            recipe: {
    ...
    const secondStep = await db.step.create({
        data: {
            action: "Moisten the rim of a chilled martini glass with the juiced lemon, turn the glass upside down, twist into the lemon sugar, and set aside.",
            stepNumber: 2,
            createdAt: new Date(),
            updatedAt: new Date(),
            recipe: {
    ...
    const thirdStep = await db.step.create({
        data: {
            action: "Add vodka, Cointreau (or triple sec), lemon juice, simple syrup, and a handful of ice to a cocktail shaker. Shake for 30 seconds or until very cold. Strain into the prepared martini glass and serve immediately.",
            stepNumber: 3,
            createdAt: new Date(),
            updatedAt: new Date(),
            recipe: {
