...
async function servingsMakeableOfRecipe(
  recipeId: string
  ...
  try {
        ...
        if (amtNeeded > recipeElement.element.quantity) {
          if (makeableServings === 1) {
            return {
              makeableServings: 0,
              error: new Error(
                `Not enough ${recipeElement.element.name} in inventory`
              ),
            };
          }
          console.log(
            `${
              recipeElement.element.name
            } only has enough in inventory to make ${
              makeableServings - 1
            } servings`
          );
    ...
    return {
      makeableServings: 0,
      error: err instanceof Error ? err : new Error(`Unknown error: ${err}`),
    };
  }
}

async function decrimentElementQuantity(
  elementId: string,
  amount: number
...
async function makeRecipeForGivenServings(
  recipeId: string,
  servings: number
): Promise<{ success: true; error: null } | { success: false; error: Error }> {
  try {
    ...
    const recipeElements = recipe.steps.flatMap((step) =>
      step.elements.map((eis) => ({
  ...
  try {
    ...
    for (const step of stepsWithIds) {
      ...
      const matchingIngredients = ingredients.filter((ing) =>
        lowerStep.includes(ing.name.toLowerCase())
      );
...
async function reupElementQuantity(
  elementId: string,
  newQuantity: number
): Promise<{ success: true; error: null } | { success: false; error: Error }> {
  try {
