...
const MakeRecipeDialog = ({
  ...
  const handleCheckServings = async () => {
    ...
    console.log(
      `Recipe ${recipe.id} can be made into ${recipeServingsPossible.makeableServings} servings`
    );
      ...
      <DialogContent>
        ...
        {recipeWarning && (
          <p className="text-red-500 text-sm mt-2 ">
            Warning: You can only make {possibleServings} servings of this
            recipe with your current inventory.
          </p>
        )}
        <div>
              ...
              <div className="flex">
                <ul className="list-disc ml-6">
                  {step.elements.map((element, idx) => (
                    <li key={idx}>
                      {element.element.name} - {element.qty * numberOfServings}{" "}
          ...
          <p className="text-sm text-muted-foreground mt-2">
            This will handle updating your inventory based on the number of
            servings
          </p>
        </div>
